...
class DatabaseManager {
    ...
    private func bindParameter(_ statement: OpaquePointer?, index: Int32, value: Any) -> Int32 {
        ...
        case let data as Data:
            return sqlite3_bind_blob(statement, index, (data as NSData).bytes, Int32(data.count), SQLITE_TRANSIENT)
        case is NSNull:
            return sqlite3_bind_null(statement, index)
        default:
            // Try to convert to string
            let stringValue = String(describing: value)
            return sqlite3_bind_text(statement, index, stringValue, -1, SQLITE_TRANSIENT)
        }
    }
