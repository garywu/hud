...
struct AnimatedFace: View {
    /// External coordinator (injected from AppDelegate)
    ...
    var body: some View {
        TimelineView(.animation) { timeline in
            ...
            Canvas { context, size in
                let expr = coordinator.currentExpression(animationTime: animationTime)
                drawFace(context: context, size: size, expression: expr, animationTime: animationTime)
            }
            .onReceive(Timer.publish(every: 0.016, on: .main, in: .common).autoconnect()) { _ in
                coordinator.updateAutoTransitions()
            }
        }
    }
...
private struct StatePreviewView: View {
    ...
    var body: some View {
        VStack {
            ...
            AnimatedFace(coordinator: coordinator, containerSize: CGSize(width: 50, height: 50))
